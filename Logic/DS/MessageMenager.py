        ...
        async for message in channelIfInteger.history(limit=1):
            if (message.author==bot.user):
                ...
                if(os.getenv("PUBLIC_CL_DS_ROLE")==''):
                    ...
                    await thread.send(f"Обновление с названием '{title}' было опубликовано. <@&{int(os.getenv("PUBLIC_CL_DS_ROLE"))}> приглашаются к ознакомлению с обновлением. Так же сообщаю что ветка успешно создана:3")
                logger.info('Thread created')
